...
| **Atelier de parcours** | Carte du territoire (projection à l'échelle, ouverte sur la zone où l'on court vraiment) : rues courues en terre cuite selon la fréquence, fond OpenStreetMap hiérarchisé (grands axes, rues, chemins) chargé par tuiles, départ habituel, échelle. **Boucles proposées sur les vraies rues** (jusqu'à trois, jamais deux fois la même, retour qui ne longe pas l'aller, curseur habitude ↔ découverte) avec « Propositions seules » pour ne voir qu'elles ; **lignes droites** de 400 m à 1,6 km près du départ, cliquables ; parcours enregistrés, points d'intérêt, export GPX. En-tête : km de rues différentes et concentration de tes km |
