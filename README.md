...
| **Atelier de parcours** | Dessiner sur son propre réseau (les rues déjà courues, sans tuiles) : boucles générées, lignes droites pour le fractionné, points d'intérêt, export GPX. Les **parcours enregistrés** s'affichent sur la carte (trait plein), avec une bascule « parcours uniquement ». La carte s'ouvre sur la zone la plus courue ; le fond de rues OpenStreetMap arrive par tuiles après l'affichage (jamais d'attente sur Overpass), en cache par zone |
